...
const LocationMovies = require('../../../models/locationsMovies');
const Knex           = require('../../../libraries/knex');

exports.create = async (payload) => {
  const payloadCopy = { ...payload };

  const name = payloadCopy.title;
  const locations = [...payloadCopy.locations];

  Reflect.deleteProperty(payloadCopy, 'title');
  Reflect.deleteProperty(payloadCopy, 'locations');

  const movie = await Movie.forge({ name }).save(payloadCopy);

  locations.map(async (location) => {
    let locationId;
    try {
      locationId = await new Location({ name: location }).fetch({ require: true });
    } catch (error) {
      locationId = await Location.forge({ name: location }).save();
    }
    await LocationMovies.forge({ location_id: locationId.id, movie_id: movie.id }).save();
  });
...
exports.updateLocations = async (request) => {
  ...
  await LocationMovies.forge({ location_id: locationId.id, movie_id: movieId }).save();
